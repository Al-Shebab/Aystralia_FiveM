...
_Prefix = GetConvar("es_prefix", '^2[Aystralia]^0')
_PrefixError = GetConvar("es_errorprefix", '^1[Aystralia]^0')
	...
	PerformHttpRequest(VersionAPIRequest, function(err, rText, headers)
		if err == 200 and rText ~= nil then
			...
			if(decoded.uuid)then
				...
				_UUID = decoded.uuid
				if(not _FirstCheckPerformed)then
					ExecuteCommand("sets AystraliaUUID " .. _UUID)
					ExecuteCommand("sets AystraliaVersion " .. _VERSION)
			...
			if not decoded.updated then
				...
				print(_Prefix .. " You're not running the newest stable version of Aystralia please update:\n" .. decoded.updateLocation)
				log('Version mismatch was detected, updater version: ' .. rText .. '(' .. _VERSION .. ')')
			else
				print(_Prefix .. " ^8Base Developer: Mailar#6308\n")
			end
			...
			if decoded.extra then
				if(show_zap ~= "1")then
					...
					if(decoded.extra ~= "^1Advertisement: ^7Want to have Aystralia pre-installed on a good and affordable server host? Go to the following link: https://zap-hosting.com/Aystralia")then
						print(decoded.extra)
					end
				end
			end
...
commands['info'].cmd = function(source, args, user)
	local Source = source
	TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "^2[^3Aystralia^2]^0 Version: ^2 " .. _VERSION)
	TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "^2[^3Aystralia^2]^0 Commands loaded: ^2 " .. (returnIndexesInTable(commands) - 2))
end
...
commands["devinfo"].cmd = function(source, args, user)
	...
	TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "^2[^3Aystralia^2]^0 Version: ^2 " .. _VERSION)
	TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "^2[^3Aystralia^2]^0 Groups: ^2 " .. (returnIndexesInTable(groups) - 1))
	TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "^2[^3Aystralia^2]^0 Commands loaded: ^2 " .. (returnIndexesInTable(commands) - 1))
	TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "^2[^3Aystralia^2]^0 Database: ^2 " .. db)
	TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "^2[^3Aystralia^2]^0 Logging enabled: ^2 " .. tostring(settings.defaultSettings.enableLogging))
end
