local options = {
    ...
    scale = 0.4,
    font = 0,
    menu_title = "Bank Manager",
    menu_subtitle = "Withdraw/Deposit Money",
}
...
function ATM()
	...
	Menu.addButton("(1,000$) Quick withdrawal", "withdrawal", nil)
	Menu.addButton("(1,000$) Quick deposit", "deposital", nil)
...
function deposit()
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        local result = GetOnscreenKeyboardResult()
       	TriggerServerEvent('bank:transfer',result)
       	--end
...
function deposital()
	TriggerServerEvent('bank:deposital', 100)
end
		...
		for k,v in ipairs(banque) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 20.0)then
				if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 5.0)then
					DisplayHelpText("~s~Press ~INPUT_PICKUP~ to access your ~r~ bank account.")
