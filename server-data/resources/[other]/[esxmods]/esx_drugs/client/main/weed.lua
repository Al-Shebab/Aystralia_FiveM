...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.RollJointCoords.RollJoint.coords, true) < 1.5 then

				
				DrawText3D( 381.888, -815.974, 29.310 , '~w~[~g~E~w~] Roll Joint')
				
				if IsControlJustReleased(0, 38)  then
					
					ESX.TriggerServerCallback('weed:getSkins', function(quantity)
						if quantity > 0 then
							ESX.TriggerServerCallback('weed:getSkins', function(quantity)
								if quantity > 0 then
									...
									exports['progressBars']:startUI(8000, "rolling joint")
									Citizen.Wait(8000)
									
									TriggerServerEvent('weed:getJoint')
									ClearPedTasksImmediately(playerPed)
								else
									ESX.ShowNotification("~r~You have no weed.")
									exports['mythic_notify']:SendAlert('error', "You have no weed.")
								end
							end, 'marijuana')	
						else
							ESX.ShowNotification("~r~You have no papers.")
							exports['mythic_notify']:SendAlert('error', "You have no papers.")
						end
					end, 'rizla')
				end
		end
