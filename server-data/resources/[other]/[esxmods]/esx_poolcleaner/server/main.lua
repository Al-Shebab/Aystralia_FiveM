...
AddEventHandler('esx_poolcleaner:GiveItem', function()
	...
	if Quantity >= 20 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'You cleaned up~g~x' .. amount .. 'swimming pool')
	end
	...
	SetTimeout(Config.Zones.Vente.ItemTime, function()
		...
		if PlayersVente[source] == true then
			...
			if Quantity < Config.Zones.Vente.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, 'Not enough facture')
				...
				TriggerClientEvent('esx:showNotification', _source, 'You got ~g~$' .. Config.Zones.Vente.ItemPrice)
				Vente(_source)
			end
...
AddEventHandler('esx_poolcleaner:startVente', function()
	...
	if PlayersVente[_source] == false then
		...
		TriggerClientEvent('esx:showNotification', _source, '~g~ Job started')
		Vente(_source)
	end
end)
