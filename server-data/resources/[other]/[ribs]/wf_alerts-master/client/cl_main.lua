...
AddEventHandler('wl-alerts:civilian:notify', function(alertType)
    street = GetTheStreet()
    local job = ESX.PlayerData.job.name -- Replace this with however you wish to check for a job
    local popo = false
    local ems = false
    if job == "police" then
    ...
    if alertType == "Suspicious" and not popo or alertType == "Suspicious" and not ambulance then
