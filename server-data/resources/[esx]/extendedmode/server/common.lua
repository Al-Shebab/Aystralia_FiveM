...
print('[Aystralia] [^2INFO^7] Starting up...')

MySQL.ready(function()
	print('[Aystralia] [^2INFO^7] Checking your database...')
	...
	MySQL.Async.fetchAll("SELECT TABLE_NAME AS 't', COLUMN_NAME AS 'c' FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'users' or TABLE_NAME = 'user_inventory' or TABLE_NAME = 'user_accounts'", {}, function(informationSchemaResult)
		local databaseCheckFunction = function()
			...
			if ExM.DatabaseType then
				...
				elseif ExM.DatabaseType == "newesx" then -- redundant check as there are no other database types but oh well, future proofing I guess
					print(("[Aystralia] [^2INFO^7] Your database is using the '%s' storage format, starting..."):format(ExM.DatabaseType))
				else
					print(("[Aystralia] [^2INFO^7] Your database is using the '%s' storage format, this is ^1not^7 compatible with ExtendedMode!"):format(ExM.DatabaseType))
					error()
				end
			else
				print("[Aystralia] [^1ERROR^7] An unknown error occured while determining your database storage format!")
				error()
			end
		end
		...
		if pcall(databaseCheckFunction) then
				...
				MySQL.Async.fetchAll('SELECT * FROM job_grades', {}, function(jobGrades)
					for k,v in ipairs(jobGrades) do
						if ESX.Jobs[v.job_name] then
							...
							print(('[Aystralia] [^3WARNING^7] Ignoring job grades for "%s" due to missing job'):format(v.job_name))
						end
					end
					...
					for k2,v2 in pairs(ESX.Jobs) do
						if ESX.Table.SizeOf(v2.grades) == 0 then
							...
							print(('[Aystralia] [^3WARNING^7] Ignoring job "%s" due to no job grades found'):format(v2.name))
						end
					end
				end)
			end)
			...
			if not ESX.StartDBSync or not ESX.StartPayCheck then
				print('[Aystralia] [^2INFO^7] Aystralia has been initialized')
			...
			print('[Aystralia] [^1ERROR^7] Aystralia was unable to intialise the database and cannot continue, please see above for more information.')
		end
	end)
end)
...
AddEventHandler('esx:clientLog', function(msg)
	if Config.EnableDebug then
		print(('[Aystralia] [^2TRACE^7] %s^7'):format(msg))
	end
end)
