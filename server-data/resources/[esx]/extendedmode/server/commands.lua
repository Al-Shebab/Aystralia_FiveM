...
ESX.RegisterCommand('save', 'admin', function(xPlayer, args, showError)
	print(('[Aystralia] [^2INFO^7] Manual player data save triggered for "%s"'):format(args.playerId.name))
	ESX.SavePlayer(args.playerId, function(rowsChanged)
		if rowsChanged ~= 0 then
			print(('[Aystralia] [^2INFO^7] Saved player data for "%s"'):format(args.playerId.name))
		else
			print(('[Aystralia] [^3WARNING^7] Failed to save player data for "%s"! This may be caused by an internal error on the MySQL server.'):format(args.playerId.name))
		end
	end)
...
ESX.RegisterCommand('saveall', 'admin', function(xPlayer, args, showError)
	print('[Aystralia] [^2INFO^7] Manual player data save triggered')
	ESX.SavePlayers(function(result)
		if result then
			print('[Aystralia] [^2INFO^7] Saved all player data')
		else
			print('[Aystralia] [^3WARNING^7] Failed to save player data! This may be caused by an internal error on the MySQL server.')
		end
	end)
