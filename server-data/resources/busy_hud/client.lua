	...
	for k, v in pairs(accounts) do
		...
		Citizen.CreateThread(function()
			if account.name == "bank" then
				...
				SendNUIMessage({action = "setBank", bank = e})			
			    else
				SendNUIMessage({action = "hideBankMoney"})
				end
			end
			while true do
				Citizen.Wait(500)
...
AddEventHandler('esx:setAccountMoney', function(account)
	...
	if account.name == "bank_money" then
		if account.money > 0 then
			SendNUIMessage({action = "setBank", bank = account.money})
		else
