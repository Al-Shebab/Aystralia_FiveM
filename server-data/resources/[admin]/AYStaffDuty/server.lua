...
local DISCORD_URL = "https://discord.com/api/webhooks/860564444030304277/pKoM9PGPkXf70tnjSadgt81TKSvpsDTnjgPJ67_us66IaTE7OMrpnOKTDJnTsKbNDPYm"
...
RegisterCommand("staffduty",function(source, args, rawCommand)
  ...
  msg = "^3 " .. Playername.." ^7is now ^8OFF DUTY" -- Sends message in chat
    ...
    sendToDiscord(StaffD, "Staff Member: **"..Playername.."** is now **OFF DUTY**", 'Time: ' .. minutesActive .. ' minutes') -- Sends message to discord
    timeTracker[source] = nil;
  local Playername = GetPlayerName(source)
  local name = "^7[ ^3Staff Duty ^7]"
  elseif IsPlayerAceAllowed(source, staff) and not has_value(people, name, time) then
    msg = "^3 " ..Playername.." ^7is now ^2ON DUTY"
        TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(41, 41, 41, 0.6); border-radius: 3px;"><i class="fas fa-globe"></i> {0}:<br> {1}</div>',
        args = { name, msg }
      })
    table.insert(people, name)
    sendToDiscord(StaffD, "Staff Member: **"..Playername.. '** is now **ON DUTY**', "Just started duty")
  end
...
AddEventHandler('playerDropped', function(reason) 
  ...
  local now = os.time();
	...
	local minutesActive = os.difftime(now, startPlusNow) / 60;
	minutesActive = math.floor(math.abs(minutesActive))
      sendToDiscord(StaffD, Playername..' is now **OFF** duty', 'Time: **' .. minutesActive .. '** minutes') -- Sends message to discord
    end
    timeTracker[source] = nil;
end)
