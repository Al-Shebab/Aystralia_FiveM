...
BanWebhookMenu = function(playerId,reason, duration)
    ...
    local discordInfo = {
        ...
        ["author"] = {
            ["name"] = 'Aystralia',
            ["url"] = '',
            ["icon_url"] = 'https://cdn.discordapp.com/attachments/802801647574843413/802865975925669929/MRP-V1.png'
        }
    }

    PerformHttpRequest(ConfigSV.KickBanWebhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://i.imgur.com/AcruQ6F.jpg", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
ScreenshotWebhook = function(playerId)
    ...
    PerformHttpRequest(ConfigSV.ScreenshotLogsWebhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://i.imgur.com/AcruQ6F.jpg", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
ReviveWebhook = function(playerId)
    ...
    local discordInfo = {
        ...
        ["description"] = '**[Admin Menu]:\n** A staff member revived another player\n**Details: **'..src..' revived '..name..'\n**Identifiers:** Steam Hex: '..steamid..', Discord: '..discord..', In-Game ID: '..playerId..' \n',
        ["footer"] = {
            ["text"] = (os.date("%B %d, %Y at %I:%M %p"))
        },
        ["author"] = {
    ...
    PerformHttpRequest(ConfigSV.ReviveWebhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://i.imgur.com/AcruQ6F.jpg", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
ForceskinWebhook = function(playerId)
    ...
    PerformHttpRequest(ConfigSV.ForceskinWebhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://i.imgur.com/AcruQ6F.jpg", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
ClearloadoutWebhook = function(playerId)
    ...
    PerformHttpRequest(ConfigSV.ClearloadoutWebhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://i.imgur.com/AcruQ6F.jpg", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
FreezeWebhook = function(playerId)
    ...
    PerformHttpRequest(ConfigSV.FreezeWebhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://i.imgur.com/AcruQ6F.jpg", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
SlapWebhook = function(playerId)
    ...
    PerformHttpRequest(ConfigSV.SlapWebhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://i.imgur.com/AcruQ6F.jpg", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
MoneyWebhook = function(playerId, money)
    ...
    PerformHttpRequest(ConfigSV.MoneyWebhook, function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://i.imgur.com/AcruQ6F.jpg", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
TriggerEvent('es:addCommand', 'report', function(source, args, user)
    if (TimeoutCommand[source] == nil or os.time() - TimeoutCommand[source] >= 10) then 
        ...
        TriggerClientEvent('chat:addMessage', -1, {
            template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 3px;"><i class="fas fa-exclamation-triangle"></i></i> {0}:<br> {1}</div>',
        ...
        TriggerClientEvent("menace:showNotification", source, "You cannot use this command for ~p~" .. (10 - (os.time() -TimeoutCommand[source])) .. "~w~ second(s).")
    end
...
PermLevelWebhook = function(playerId, group)
    ...
    local discordInfo = {
        ["color"] = "65793",
        ["type"] = "rich",
        ["title"] = "Player Perms Set",
        ["description"] = '**[Admin Menu]:\n** A staff member set perms for another player\n**Details: **'..src..' set '..name..' to '..group..'| **Identifiers:** Steam Hex: '..steamid..', Discord: '..discord..', In-Game ID: '..playerId..' \ndiscord.gg/menacerp/',
        ["footer"] = {
            ["text"] = (os.date("%B %d, %Y at %I:%M %p"))
        },
        ["author"] = {
    ...
    PerformHttpRequest("https://discord.com_/api/webhooks/798306127926132776/Usrw048ByvkGceYHqrReA24hjKBOCSjPmxHsVBOR5IoS1jvZZF1n8K32ZJiqKzpf6i0h", function(err, text, headers) end, 'POST', json.encode({ username = 'Admin System', avatar_url = "https://cdn.discordapp.com/attachments/751290121889775697/797195339131781190/MenaceRPLogo10.gif", embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
TriggerEvent('es:addGroupCommand', 'announce', "admin", function(source, args, user)
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(148, 0, 211, 0.6); border-radius: 3px;"><i class="fas fa-cog"></i> ANNOUNCEMENT:<br> {1}<br></div>',
...
AddEventHandler('venomadmin:revive', function(playerId)
    if playerId ~= nil then
        if IsPlayerAceAllowed(source, "venomadmin.revive") then
            ...
            TriggerClientEvent("menace_ambulancejob:7774831", playerId)
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You do not have permission to do this.', length = 5000})
        end
    else
...
AddEventHandler('venomadmin:forceskin', function(playerId)
    if playerId ~= nil then
        if IsPlayerAceAllowed(source, "venomadmin.forceskin") then
            ...
            TriggerClientEvent("menace_skin:openSaveableMenu", playerId)
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You do not have permission to do this.', length = 5000})
        end
    else
