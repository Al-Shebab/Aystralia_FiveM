...
ESX.RegisterCommand = function(name, group, cb, allowConsole, suggestion)
	...
	if ESX.RegisteredCommands[name] then
		print(('[Aystralia] [^3WARNING^7] An command "%s" is already registered, overriding command'):format(name))
	...
	RegisterCommand(name, function(playerId, args, rawCommand)
		...
		if not command.allowConsole and playerId == 0 then
			print(('[Aystralia] [^3WARNING^7] %s'):format(_U('commanderror_console')))
		else
			...
			if error then
				if playerId == 0 then
					print(('[Aystralia] [^3WARNING^7] %s^7'):format(error))
				else
				...
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						print(('[Aystralia] [^3WARNING^7] %s^7'):format(msg))
					else
...
ESX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[Aystralia] [^3WARNING^7] Server callback "%s" does not exist. Make sure that the server sided file really is loading, an error in that file might cause it to not load.'):format(name))
	end
end
...
ESX.StartDBSync = function()
	function saveData()
		ESX.SavePlayers(function(result)
			if result then
				print('[Aystralia] [^2INFO^7] Automatically saved all player data')
			else
				print('[Aystralia] [^3WARNING^7] Failed to automatically save player data! This may be caused by an internal error on the MySQL server.')
			end
		end)
