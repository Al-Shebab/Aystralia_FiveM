...
function loadESXPlayer(identifier, playerId)
	...
	MySQL.Async.fetchAll('SELECT accounts, job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
		...
		if ESX.DoesJobExist(job, grade) then
			...
			print(('[Aystralia] [^3WARNING^7] Ignoring invalid job for %s [job: %s, grade: %s]'):format(identifier, job, grade))
		...
		if result[1].inventory and result[1].inventory ~= '' then
			...
			for name,count in pairs(inventory) do
				...
				if item then
					...
					print(('[Aystralia] [^3WARNING^7] Ignoring invalid item "%s" for "%s"'):format(name, identifier))
				end
			end
		end
		...
		if result[1].position and result[1].position ~= '' then
			...
			print('[Aystralia] [^3WARNING^7] Column "position" in "users" table is missing required default value. Using backup coords, fix your database.')
...
-- Add support for Aystralia >6.4.x
