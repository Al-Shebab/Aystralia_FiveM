...
realisticVehicleFailurecfg = {
	...
	DamageMultiplier = 2.5,						-- you may edit this to your liking. if "chargeForRepairs = false" ignore this one
...
Citizen.CreateThread(function()
	if (realisticVehicleFailurecfg.displayBlips == true) then
		for _, item in pairs(repairLocationsCfg.mechanics) do
			tvRP.addBlip(item.x, item.y, item.z,item.id,4,"Mechanic",0.4)
		end
	end
end)
