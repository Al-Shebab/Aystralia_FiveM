...
function vRP.getPlayerEndpoint(player)
  return GetPlayerEP(player) or "Failed"
end

function vRP.getPlayerName(player)
  return GetPlayerName(player) or "Failed"
end
...
function vRP.ban(source,reason)
  ...
  if user_id ~= nil then
    ...
    vRP.kick(source,"[ELRP] | Permanent Ban | Your ID: "..user_id.." | Reason: "..reason.." | Appeal @ el-rp.uk")
  end
end
...
function vRP.kick(source,reason)
  local user_id = vRP.getUserId(source)
  if user_id ~= nil then
  DropPlayer(source,"[ELRP] | Kicked | Your ID: "..user_id.." | Reason: "..reason..".")
  end
end
...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
  ...
  local ids = GetPlayerIdentifiers(source)
  if ids ~= nil and #ids > 0 then
    deferrals.update("[ELRP] Checking identifiers...")
    vRP.getUserIdByIdentifiers(ids, function(user_id)
      if user_id ~= nil then 
        deferrals.update("[ELRP] Checking banned...")
        vRP.isBanned(user_id, function(banned)
          if not banned then
            deferrals.update("[ELRP] Checking whitelisted...")
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then 
                  ...
                  deferrals.update("[ELRP] Loading datatable...")
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    local data = json.decode(sdata)
                    if type(data) == "table" then vRP.user_tables[user_id] = data end
                    local tmpdata = vRP.getUserTmpTable(user_id)
                    deferrals.update("[ELRP] Getting last login...")
                    vRP.getLastLogin(user_id, function(last_login)
                      ...
                      print("[ELRP] [Name: "..name.."] [IP: "..vRP.getPlayerEndpoint(source).."] Joined [PermID: "..user_id.."]")
                      TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                      deferrals.done()
                    end)
                  end)
                else 
                  print("[ELRP] [Name: "..name.."] [IP: "..vRP.getPlayerEndpoint(source).."] Re-Joined [PermID: "..user_id.."]")
                ...
                print("[ELRP] [Name: "..name.."] [IP: "..vRP.getPlayerEndpoint(source).."] Rejected: NOT Whitelisted [PermID: "..user_id.."]")
                deferrals.done("[ELRP] | Rejected: Error [1].")
              end
            end)
          else
            print("[ELRP] [Name: "..name.."] [IP: "..vRP.getPlayerEndpoint(source).."] Rejected: Banned [PermID: "..user_id.."]")
            deferrals.done("[ELRP] | Permanent Ban | Your ID: "..user_id.." | Appeal @ el-rp.uk")
          end
        end)
      else
        print("[ELRP] [Name: "..name.."] [IP: "..vRP.getPlayerEndpoint(source).."] Rejected: Identification Error [PermID: "..user_id.."]")
        deferrals.done("[ELRP] | Rejected: Error [2].")
      end
    end)
  else
    print("[ELRP] [Name: "..name.."] [IP: "..vRP.getPlayerEndpoint(source).."] Rejected: Identification Error [PermID: "..user_id.."]")
...
AddEventHandler("playerDropped",function(reason)
  ...
  if user_id ~= nil then
    ...
    print("[ELRP] [IP: "..vRP.getPlayerEndpoint(source).."] Disconnected [PermID: "..user_id.."]")
...
AddEventHandler("vRPcli:playerSpawned", function()
  ...
  if user_id ~= nil then
    ...
    SetTimeout(2000, function()
      ...
      SetTimeout(config.load_duration*0, function()
