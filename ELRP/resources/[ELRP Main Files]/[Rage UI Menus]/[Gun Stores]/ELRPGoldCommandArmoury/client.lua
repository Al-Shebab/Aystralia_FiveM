...
RMenu.Add('vrpgunshops', 'weapons', RageUI.CreateSubMenu(RMenu:Get('vrpgunshops', 'main'), "", "~b~Special", nil, nil, "pdarmoury", "pdarmoury"))
...
function weaponsBtn() 
    RageUI.Button("Special" , nil, {
        RightLabel = "→→→",
    }, true, function(Hovered, Active, Selected)
        if Selected then
            -- My action the button is selected
        end
    end, RMenu:Get('vrpgunshops', 'weapons'))
end
...
function weaponsSM() 
    RageUI.IsVisible(RMenu:Get('vrpgunshops', 'weapons'), true, false, true, function()
        for i , p in pairs(cfg.guns.weapons) do 
